...
module.exports = function (options) {
  ...
  var iPromise = new Promise(function (res, rej) {

    if (options.removePath === true) {
      var unlink = function (sftp, path) {
          ...
          if (sftp) {
            sftp.unlink(path, function (err) {
              if (err) {
                ...
                gutil.log("Removed: ", gutil.colors.green(path));
                resolve();
              }
            });
          } else {
          ...
          if (sftp) {
            sftp.readdir(path, function (err, list) {
              if (err) {
                ...
                gutil.log('err: ', gutil.colors.red(err));
                reject(err);
      ...
      var rmdir = function (sftp, path, recursive) {
        ...
        return new Promise(function (resolve, reject) {
          if (sftp) {
            ...
            var rmdir = function (p) {
              return list(sftp, p).then(function (list) {
                if (list.length > 0) {
                  var promises = [];
                  ...
                  return new Promise(function (resolve, reject) {
                    return sftp.rmdir(p, function (err) {
                      if (err) {
                        gutil.log(gutil.colors.red(err));
                        reject(err);
                      } else {
            ...
            return rmdir(path).then(function () {
              gutil.log("Removed: ", gutil.colors.green(path));
              resolve()
            })
              .catch(function (err) {
      ...
      pool.call(this, remotePath, function (sftp) {
        rmdir(sftp, remotePath, true).then(res, rej);
      });
    } else {
      res();
    }
  ...
  return through.obj(
    function (file, enc, cb) {
      ...
      iPromise.then(theResponse.bind(this), theResponse.bind(this));
    },
    function (cb) {
      if (fileCount > 0) {
